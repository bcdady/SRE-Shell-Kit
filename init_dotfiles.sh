...
if ((installerrun)); then
  eval "$(git config user.name >&/dev/null && git config user.email >&/dev/null)"
...
if ((getgitinfo)); then
  eval "$(git config user.name >&/dev/null)"
  if [[ $? -ne 0 ]]; then
    gitname="Donald Knuth"
  else
    gitname="$(git config user.name)"
  fi
  eval "$(git config user.email >&/dev/null)"
  if [[ $? -ne 0 ]]; then
